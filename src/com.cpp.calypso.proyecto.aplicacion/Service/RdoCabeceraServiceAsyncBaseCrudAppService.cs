...
using com.cpp.calypso.proyecto.aplicacion.Models;
...
{
    public class RdoCabeceraAsyncBaseCrudAppService : AsyncBaseCrudAppService<RdoCabecera, RdoCabeceraDto, PagedAndFilteredResultRequestDto>, IRdoCabeceraAsyncBaseCrudAppService
    {
        private readonly IBaseRepository<RdoDetalleEac> _detalleRepository;
        ...
        public RdoCabeceraAsyncBaseCrudAppService(
            IBaseRepository<RdoCabecera> repository,
            IBaseRepository<RdoDetalleEac> detalleRepository,
        ...
        public RdoCabecera GetDetalles(int RdoCabeceraId)
        {
            var rdo = Repository.Get(RdoCabeceraId);
            return rdo;
        }

        public List<RdoCabecera> GetRdoCabeceras(int ProyectoId)
        {
        ...
        public List<RdoCabeceraDto> GetRdoCabecerasTable(int ProyectoId)
        {
            var rdoQuery = Repository.GetAllIncluding(c => c.Proyecto, c => c.Proyecto.Contrato, c => c.Proyecto.Contrato.Cliente)
                .Where(e => e.vigente == true)
                .Where(c => c.ProyectoId == ProyectoId)
        ...
        public int CreateRdoCabecera(int ProyectoId, DateTime fecha)
        {
        ...
        {
            var query = _eacRepository.GetAllIncluding(o => o.Computo, o => o.RdoCabecera.Proyecto)
               .Where(o => o.vigente)
               .Where(o => o.RdoCabeceraId == RdoCabeceraId).ToList();
            ...
            excelPackage.Workbook.Properties.Title = "RDO";
            excelPackage.Workbook.Properties.Subject = "Generación de RDO";
            excelPackage.Workbook.Properties.Created = DateTime.Now;


            //Crea una hoja de trabajo
            ExcelWorksheet worksheet = excelPackage.Workbook.Worksheets.Add("RDO");
            ...
            worksheet.Cells["C4:AG4"].Value = proyecto.es_RSO ? "REPORTE SEMANAL OBRA" : "REPORTE DIARIO OBRA";
            ...
            worksheet.Cells["C5:AG5"].Value = "(" + query[0].RdoCabecera.Proyecto.codigo + ") " + query[0].RdoCabecera.Proyecto.descripcion_proyecto;
            ...
            worksheet.Cells["C6:AG6"].Value = query[0].RdoCabecera.fecha_rdo;
            ...
            worksheet.Cells["Q13:Q14"].Value = "AC Diario";
            ...
            worksheet.Cells["T13:T14"].Value = "EV Diario";
            ...
            worksheet.Cells["AB14"].Value = "Diaria";
            ...
            worksheet.Cells["AE13:AE14"].Value = "% Avance Diario";
        ...
        public ExcelPackage GenerarExcelRdo(int RdoCabeceraId, string TipoReporte)
        {
            ...
            if (second_format) //Si es Segundo Formato
            {
                var rdoDetalles = _eacRepository.GetAll()
                                                            .Where(c => c.vigente)
                                                           .Where(c => c.RdoCabeceraId == cabecera.Id)
                                                            .ToList();
            ...
            var rdoDetallesAdicionales = _eacRepository.GetAll()
                                                   .Where(c => c.vigente)
                                                   .Where(c => c.RdoCabeceraId == RdoCabeceraId)
            ...
            if (filaAdicionales > 0)
            {
                ...
                var fechatemp = ma.fecha_inicio_real != new DateTime(1999, 01, 01) && ma.fecha_inicio_real != null ? "" + ma.fecha_inicio_real.Value.ToString("dd-MMM-yy") : "";
        ...
        .Copy(worksheet.Cells[1, 46, worksheet.Dimension.End.Row, 46]);
            ...
            // excelPackage.Workbook.Worksheets.Delete(hojaAdicionales);
            return excelPackage;
        }
        ...
        public List<RdoDatos> GetRdoDetalles(int WbsId, int cabeceraId)
        {
            ...
            var query = _detalleRepository.GetAllIncluding(o => o.Computo)
                ...
                .Where(o => o.RdoCabeceraId == cabeceraId)
        ...
        public List<RdoCurva> ActualizarCurva(int ProyectoId, int RdoCabeceraId)
        {
            ...
            var rdo_detalles = _detalleRepository.GetAllIncluding(c => c.RdoCabecera, c => c.Computo)
                                                    .Where(c => c.RdoCabeceraId == RdoCabeceraId)
                                                    .Where(c => c.vigente)
            ...
            for (DateTime fecha = fecha_inicial; fecha <= fecha_final; fecha = fecha.AddDays(1))
            {
                ...
                var valores_reales = _detalleRepository.GetAllIncluding(c => c.RdoCabecera, c => c.Computo)

                                                            .Where(c => c.RdoCabecera.es_definitivo)
                                                            .Where(c => c.RdoCabecera.fecha_rdo == fecha)
                                                           .Where(c => c.vigente)
        ...
        public ExcelPackage MontoTotales(int Id, string TipoReporte)
        {
            ...
            var detallesRDOAnterior = new List<RdoDetalleEac>();

            if (rdoAnterior != null)
            {

                var list = _detalleRepository.GetAll().Where(c => c.vigente)
                                            .Where(c => c.RdoCabeceraId == rdoAnterior.Id)
                                            .ToList();
            ...
            for (DateTime fecha = fechaInicio; fecha <= fecha_final; fecha = fecha.AddDays(1))
            {
                ...
                foreach (var i in rdoDetalles)
                {
                    ...
                    //     valor_acum = valor_acum + Decimal.Round(valor, 8);
                    valor_acum = valor_acum + valor;

                }


                //  valor_previsto_total = Decimal.Round(valor_previsto_total + valor_acum, 8);
                ...
                var ValoresCurvas = (from l in ValoresRealesCurva where l.fecha == fecha where l.valor_real_acumulado > 0 select l).FirstOrDefault();
                ...
                if (ValoresCurvas != null && ValoresCurvas.valor_real_acumulado > 0)
                {
                    valor_real_acumulado = ValoresCurvas.valor_real_acumulado;
                }
        ...
        {
            var detalle = _detalleRepository.GetAll().Where(c => c.vigente).Where(c => c.RdoCabeceraId == RdoCabeceraId).ToList();
            if (detalle.Count > 0)
            {
        ...
        public ExcelPackage GenerarErroresCantidadesRDOS()
        {
            ExcelPackage package = new ExcelPackage();
            var workbook = package.Workbook;
            var h = workbook.Worksheets.Add("CANTIDADES_ERRADAS");

            var rdo = Repository.GetAll().Where(c => c.es_definitivo)
                                         .Where(c => c.vigente)
                                         .OrderByDescending(c => c.fecha_rdo)
                                         .ToList();

            var agrupado = (from t in rdo
                            orderby t.fecha_rdo descending
                            group t by new
                            {
                                t.ProyectoId,
                            }
            ...
            cell = "B" + cabecera;
            h.Cells[cell].Value = "CANTIDAD EAC";
            cell = "C" + cabecera;
            h.Cells[cell].Value = "CANTIDAD ANTERIOR";
            cell = "D" + cabecera;
            h.Cells[cell].Value = "ITEM CODIGO";
            cell = "E" + cabecera;
            h.Cells[cell].Value = "ITEM NOMBRE";
            cell = "F" + cabecera;
            h.Cells[cell].Value = "FECHA INICIAL";
            cell = "G" + cabecera;
            h.Cells[cell].Value = "FECHA FINAL";
            cell = "H" + cabecera;
            h.Cells[cell].Value = "FECHA RDO";
            foreach (var a in agrupado)
            {
                var proyecto = _proyectoRepository.Get(a.Grupo.ProyectoId);
                var rdoCabecera = Repository.Get(a.Definitivo.Id);
                var detallesErroneos = _detalleRepository.GetAll().Where(c => c.RdoCabeceraId == rdoCabecera.Id)
                                                                  .Where(c => c.cantidad_eac < c.cantidad_anterior)
                                                                  .ToList();


                foreach (var d in detallesErroneos)
                {
                    var WBS = _wbsRepository.Get(d.WbsId);
                    cell = "A" + fila;
                    h.Cells[cell].Value = proyecto.codigo;

                    cell = "B" + fila;
                    h.Cells[cell].Value = d.cantidad_eac;


                    cell = "C" + fila;
                    h.Cells[cell].Value = d.cantidad_anterior;

                    cell = "D" + fila;
                    h.Cells[cell].Value = d.codigo_preciario;
                    cell = "E" + fila;
                    h.Cells[cell].Value = d.nombre_actividad;
                    cell = "F" + fila;
                    h.Cells[cell].Value = d.fecha_inicio_prevista.GetValueOrDefault();

                    cell = "G" + fila;
                    h.Cells[cell].Value = d.fecha_fin_prevista.GetValueOrDefault();
                    cell = "H" + fila;
                    h.Cells[cell].Value = rdoCabecera.fecha_rdo.ToShortDateString();

                    var wbs = _wbsRepository.GetAll().Where(c => c.Id == d.WbsId).FirstOrDefault();



                    List<Wbs> Jerarquia = new List<Wbs>();

                    Wbs item = _wbsRepository.Get(d.WbsId);
                    Jerarquia.Add(item);
                    while (item.id_nivel_padre_codigo != ".")
                    {
                        ...
                        item = _wbsRepository.GetAll()
                            ...
                            .Where(C => C.vigente)
                             .Where(C => C.OfertaId == wbs.OfertaId)
                            .FirstOrDefault();
                    ...
                    //ColumnasWBS
                    int columnaInicial = 9; //columnas Inicial Jerarquia

                    foreach (var itemWbs in Jerarquia) //.OrderBy(c=>c.Id))
                    {

                        h.Cells[fila, columnaInicial].Value = itemWbs.nivel_nombre;
                        columnaInicial++;
                    }
        ...
        public ExcelPackage GenerarErroresCantidadesCero()
        {
            ExcelPackage package = new ExcelPackage();
            var workbook = package.Workbook;
            var h = workbook.Worksheets.Add("CANTIDADES_ERRADAS");
        ...
        public bool ExisteRdoGeneradoSuperior(int ProyectoId, DateTime fechageneracion)
        {
            var avanceDate = fechageneracion.Date;
            var ExisteRdoGenerado = Repository.GetAll()
                                                      .Where(c => c.es_definitivo)
            ...
            return ExisteRdoGenerado != null ? true : false;
        }
