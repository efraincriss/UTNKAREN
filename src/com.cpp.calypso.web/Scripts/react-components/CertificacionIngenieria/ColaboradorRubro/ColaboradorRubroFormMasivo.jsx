...
export class ColaboradorRubroFormMasivo extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      data: {
        ...
        ItemId: props.rubro.ItemId ? props.rubro.ItemId : 0,
  ...
  componentWillReceiveProps(prevProps) {
    let updatedValues = {
      ...
      ItemId: prevProps.rubro.ItemId ? prevProps.rubro.ItemId : 0,
        ...
        <div className="row">
          <div className="col">
            <Field
              ...
              label="Item"
              type="select"
              options={this.props.catalogoItems}
              edit={true}
              readOnly={false}
              filter={true}
              value={this.state.data.ItemId}
              onChange={this.onChangeItem}
              error={this.state.errors.ItemId}
  ...
  isValid = () => {
    ...
    let itemId = this.state.data.ItemId
    ...
    if ( itemId === 0) {
      errors.ItemId = "El Item es requerido"
    }
  ...
  onChangeItem = (name, value) => {
    ...
    this.comprobarExistenciaItemEnContrato(value);
  }

  comprobarExistenciaItemEnContrato = (itemId) => {
    this.props.blockScreen()
    let url = ""
    url = `${config.apiUrl}${MODULO_CERTIFICACION_INGENIERIA}/${CONTROLLER_COLABORADOR_RUBRO}/ComprobarExistenciaItemEnContrato/${itemId}`
    console.log(url)
    http
      .get(url)
      .then((response) => {
        ...
        if (data.Valido !== true) {
          var message = data.Mensajes.join();
          this.showWarn(message)
          this.props.unlockScreen()
        }
        this.props.unlockScreen()
