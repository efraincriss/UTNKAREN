...
export default class EditarUsuarioExterno extends React.Component {
                    ...
                    <div className="row">
                            ...
                            <div className="row">
                                ...
                                <div className="col">
                                    <div className="form-group">
                                        ...
                                        <input type="text" id="nro_identificacion" className="form-control" value={this.state.nro_identificacion} onChange={this.handleChange} name="nro_identificacion" required />
                      ...
                      />)}
                        ...
                        <div className="col" style={{ display: 'inline-block' }}>
                            <div className="row">{this.convertirimagen(this.state.fotografia)}</div>
                            <div className="row"><label>{this.state.msg}</label></div>
                        </div>
    ...
    ConsultaUsuarioExterno() {
        axios.post("/RRHH/Colaboradores/GetUsuarioExternoApi/", { id: sessionStorage.getItem('id_usuario_externo') })
            .then((response) => {
                ...
                this.setState({
                    ...
                    fecha_nacimiento: moment(response.data.fecha_nacimiento).format("YYYY-MM-DD"),
                    genero: response.data.catalogo_genero_id,
                    nacionalidad: response.data.pais_pais_nacimiento_id,
                    pais: response.data.PaisId,
                    estado_civil: response.data.catalogo_estado_civil_id,
                    fecha_matrimonio: response.data.fecha_matrimonio != null ? moment(response.data.fecha_matrimonio).format("YYYY-MM-DD") : '',
                    telefono: response.data.Contacto.celular,
                    email: response.data.Contacto.correo_electronico,
                    id_colaborador: response.data.Id,
                    id_contacto: response.data.ContactoId,
                    visible_fecha_matrimonio: response.data.fecha_matrimonio != null ? true : false,
                    codigo_dactilar: response.data.codigo_dactilar != null ? response.data.codigo_dactilar : '',
                    disable_consumo: response.data.viene_registro_civil == true ? true : false,
                    disable_sexo: response.data.viene_registro_civil == true ? true : false,
                    disable_estado_civil: response.data.viene_registro_civil == true ? true : false,
                    disable_nacionalidad: response.data.viene_registro_civil == true ? true : false,
                    disable_fecha_matrimonio: response.data.viene_registro_civil == true ? true : false,
                    viene_registro_civil: response.data.viene_registro_civil,
                    fecha_registro_civil: response.data.fecha_registro_civil != null ? moment(response.data.fecha_registro_civil).format("YYYY-MM-DD HH:mm:ss") : '',
                    msg: response.data.viene_registro_civil != true ? '' : "Información de Registro Civil actualizada " + moment(response.data.fecha_registro_civil).format("YYYY-MM-DD HH:mm:ss"),
                    grupo_personal: response.data.catalogo_grupo_personal_id,
                    es_visita:response.data.es_visita
                })
                this.saveEdad();
                this.getFotografia();
            })
    ...
    getFotografia() {
        ...
        axios.post("/RRHH/Colaboradores/GetArchivoFotografiaApi/",
            {
                Idcolaborador: this.state.id_colaborador,
                origen: origen
            })
            .then((response) => {
    ...
    titulo() {
        ...
        p.innerHTML = ' Administración de Colaborador Externo';
    ...
    handleValidation() {
        ...
        if (!this.state.fecha_nacimiento) {
            ...
            if (this.state.edad < 15) {
                this.state.formIsValid = false;
                errors["fecha_nacimiento"] = "El usuario no puede ser menor de 15 años";
            }
        }
        if (!this.state.pais) {
            this.state.formIsValid = false;
    ...
    Guardar() {
        this.setState({ loading: true });
        axios.post("/RRHH/Colaboradores/EditUsuarioExterno/", {
            Id: this.state.id_colaborador,
            ContactoId: this.state.id_contacto,
            ...
            catalogo_grupo_personal_id: this.state.grupo_personal,
