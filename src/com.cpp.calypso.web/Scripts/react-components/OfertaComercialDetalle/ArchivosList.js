...
import { Growl } from "primereact-v2/components/growl/Growl";
 export default class  ArchivosList extends React.Component {
    ...
    ObtenerArchivos = () => {
        ...
        axios
            .post("/Proyecto/OfertaComercial/UploadFileList/" +  document.getElementById("OfertaComercialId").className, {})
            .then(response => {
    ...
    Eliminar = (Id) => {
        console.log(Id);
        axios
            .post("/Proyecto/OfertaComercial/DeleteArchivo", {
                Id: Id
            })
            .then(response => {
                if (response.data == "OK") {
    ...
    onDownloaImagen = Id => {
        ...
        return (
            window.location = `/Proyecto/OfertaComercial/DescargarArchivoOrden/${Id}`
        );
    }
    ...
    EnviarFormulario(event) {
        ...
        if (!this.isValid()) {
            ...
            if (this.state.action == 'create') {
                ...
                formData.append('Id',  document.getElementById("OfertaComercialId").className);
                ...
                axios
                    .post("/Proyecto/OfertaComercial/CreateArchivo",formData ,multipart)
                    .then(response => {
