...
export default class ColaboradorTable extends React.Component {
                ...
                <div className="col">
                    <BootstrapTable data={this.props.data} hover={true} pagination={true}>
                        ...
                        <TableHeaderColumn
                            ...
                            thStyle={{ whiteSpace: 'normal', textAlign: 'center' }}
                            filter={{ type: 'TextFilter', delay: 500 }}
                        >Tipo Identificación</TableHeaderColumn>
        ...
        if (row.tiene_derecho === 'SI') {
            return (
                <div>
                    ...
                    <button className="btn btn-outline-info" data-toggle="tooltip" data-placement="left" title="Crear Reserva"
                        onClick={
                            () => { if (window.confirm(`Se creará la reseva, Desea continuar?`)) this.submit(cell); }
                        }>
    ...
    submit = (cell) => {
        this.props.seleccionarColaborador(cell)
    }
    activarservicio = (row) => {
