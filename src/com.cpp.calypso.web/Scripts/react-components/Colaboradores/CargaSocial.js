...
import {
    TIPO_IDENTIFICACION_PASAPORTE, TIPO_IDENTIFICACION_CEDULA,
    ESTADO_CIVIL_CASADO, ESTADO_CIVIL_SOLTERO, ESTADO_CIVIL_DIVORCIADO,
    REGISTRO_CIVIL_CASADO, REGISTRO_CIVIL_SOLTERO, REGISTRO_CIVIL_DIVORCIADO,
    GENERO_MUJER, GENERO_VARON, REGISTRO_CIVIL_MUJER, REGISTRO_CIVIL_HOMBRE,
    NACIONALIDAD_ECUATORIANA, REGISTRO_CIVIL_ECUATORIANA, PARENTESCO_HIJO,
    PARENTESCO_CONYUGE, PARENTESCO_CONVIVIENTE
} from './Codigos';
import { BootstrapTable, TableHeaderColumn } from 'react-bootstrap-table';

export default class CargaSocial extends React.Component {
    ...
    constructor(props) {
        ...
        this.getFormSelectPorcentaje = this.getFormSelectPorcentaje.bind(this);
    ...
    abrirRegistroCargaNuevo() {
        this.setState({ visible: true, createstate: true, editstate: false, viene_registro_civil: false });
    ...
    consultaBDD() {
        ...
        if (this.state.consIsValid == true) {
            ...
            axios.post("/RRHH/Colaboradores/GetInfoCargaSocialWSApi", {
                ...
                .then((response) => {
                    if (response.data == "NO") {
                        ...
                        this.props.successMessage(this.state.nro_identificacion + " Encontrado en base de datos");
                        this.procesarDatosBDD(response.data);
                        if (response.data.ColaboradoresId != this.props.id_colaborador) {
                            this.props.warnMessage("Familiar se encuentra registrado con el Colaborador " +
    ...
    consultaPublica() {
        axios.post("/Accesos/ConsultaPublica/ExisteCandidato", {
            ...
            axios.post("/RRHH/Colaboradores/Consumir/", {
                ...
                .then((response) => {
                    if (response.data === "SIN_RESPUESTA") {
                        this.props.warnMessage("Sin respuesta al Web Service verifique los datos de conección")
                    } else {
    ...
    procesarDatosConsumo(datos) {
        ...
        if (datos.EstadoCivil == REGISTRO_CIVIL_CASADO) {
            ...
            var estado = this.state.estados.filter(c => c.codigo == ESTADO_CIVIL_CASADO);
            this.setState({
            ...
            if (datos.EstadoCivil == REGISTRO_CIVIL_DIVORCIADO) {
                var estado = this.state.estados.filter(c => c.codigo == ESTADO_CIVIL_DIVORCIADO);
                this.setState({ estado_civil: estado[0].Id, disable_estado_civil: true })
            } else if (datos.EstadoCivil == REGISTRO_CIVIL_SOLTERO) {
                var estado = this.state.estados.filter(c => c.codigo == ESTADO_CIVIL_SOLTERO);
                this.setState({ estado_civil: estado[0].Id, disable_estado_civil: true })
            } else {
        ...
        if (datos.Nacionalidad == REGISTRO_CIVIL_ECUATORIANA) {
            var n = this.state.tiposNacionalidades.filter(c => c.codigo == NACIONALIDAD_ECUATORIANA);
    ...
    procesarConsultaPublica(datos) {
        ...
        if (datos.estado_civil == REGISTRO_CIVIL_CASADO) {
            ...
            this.setState({
                estado_civil: estado[0].Id,
                disable_estado_civil: true,
                fecha_matrimonio: datos.fecha_matrimonio == "" ? '' : moment(fecha_m).format("YYYY-MM-DD"),
                visible_fecha_matrimonio: true,
                disable_fecha_matrimonio: datos.fecha_matrimonio == "" ? false : true
            })
        } else
            if (datos.estado_civil == REGISTRO_CIVIL_DIVORCIADO) {
                var estado = this.state.estados.filter(c => c.codigo == ESTADO_CIVIL_DIVORCIADO);
                this.setState({ estado_civil: estado[0].Id, disable_estado_civil: true })
                ...
                this.setState({ estado_civil_sugerido: datos.estado_civil, disable_estado_civil: false, estado_civil: '' })
            }
    ...
    validationConsumo() {
        ...
        if (!this.state.tipo_identificacion) {
            ...
            errors["tipo_identificacion"] = <div className="alert alert-danger">El campo Tipo de Identificación es obligatorio.</div>;
        } else {
            ...
            if (catalogo[0].codigo != 'CEDULA' && this.state.es_bdd == false) {
                this.state.consIsValid = false;
                errors["tipo_identificacion"] = <div className="alert alert-danger">Consulta se realiza con CEDULA</div>;
            }
        }
        if (!this.state.nro_identificacion) {
            ...
            if (!isFinite(this.state.nro_identificacion)) {
                this.state.consIsValid = false;
                errors["nro_identificacion"] = <div className="alert alert-danger">El campo permite solo ingreso numérico</div>;
            }
            if (this.state.nro_identificacion.length != 10) {
                this.state.consIsValid = false;
                errors["nro_identificacion"] = <div className="alert alert-danger">El campo debe tener 10 dígitos</div>;
            }
    ...
    render() {
        const footer = (
            <div>
                ...
                <Button label="Cancelar" icon="pi pi-times" onClick={this.cerrarRegistroCarga} className="p-button-secondary" />
            </div>
        );
        const options = {
        ...
        return (
            <BlockUi tag="div" blocking={this.state.loading}>
                <div>
                    <div className="row">
                        <div className="col-xs-12 col-md-12">
                            <form onSubmit={this.handleSubmit}>
                                <div className="row">
                                    <div className="col" style={{ visibility: this.props.colaborador.es_sustituto == true && this.state.es_sustituto == false ? 'visible' : 'hidden' }}>
                                        <div className="alert alert-danger">El colaborador tiene sustituto, recuerde registrarlo</div>
                                    </div>
                                    <div className="col">
                                        <button type="button" onClick={this.abrirRegistroCargaNuevo} className="btn btn-outline-primary pull-right">Agregar Familiar</button>
                                    </div>
                                </div>
                                <br />
                                <div className="row">
                                    <div className="col">
                                        ...
                                        >
                                            ...
                                            {/* <TableHeaderColumn width={'6%'} dataField='nro' isKey={true} dataAlign="center" headerAlign="center" dataAlign="center" dataSort={true}>No.</TableHeaderColumn> */}
                                            <TableHeaderColumn dataField="tipoIdentificacion" thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataSort={true}>Tipo Identificación</TableHeaderColumn>
                                            <TableHeaderColumn dataField='nro_identificacion' isKey={true} thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataSort={true}>Número Identificación</TableHeaderColumn>
                                            <TableHeaderColumn width={'15%'} dataField="apellidos" thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataSort={true}>Apellidos</TableHeaderColumn>
                                            <TableHeaderColumn width={'15%'} dataField="nombres" thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataSort={true}>Nombres</TableHeaderColumn>
                                            <TableHeaderColumn dataField='parentesco' thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataSort={true}>Parentesco</TableHeaderColumn>
                                            <TableHeaderColumn dataField="fecha_nacimiento" thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataAlign="center" dataSort={true}>Fecha de Nacimiento</TableHeaderColumn>
                                            <TableHeaderColumn dataField="nombre_genero" thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataAlign="center" dataSort={true}>Género</TableHeaderColumn>
                                            <TableHeaderColumn dataField="nombre_dis" thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataAlign="center" dataSort={true}>Discapacidad</TableHeaderColumn>
                                            <TableHeaderColumn dataField="nombre_sustituto" thStyle={{ whiteSpace: 'normal' }} tdStyle={{ whiteSpace: 'normal' }} headerAlign="center" dataAlign="center" dataSort={true}>Por Sustitución</TableHeaderColumn>
                                            <TableHeaderColumn width={'13%'} dataField='Operaciones' dataFormat={this.generateButton.bind(this)} headerAlign="center" dataAlign="center">Opciones</TableHeaderColumn>
                                        </BootstrapTable>
                                    </div>
                                </div>
                    ...
                    <Dialog header="Registro de Familiares" visible={this.state.visible} style={{ width: '50%' }} minY={70} footer={footer} onHide={this.cerrarRegistroCarga} maximizable={true}>

                        <div className="row">
                            <div className="col">
                                <div className="form-group">
                                    <label htmlFor="label">* Tipo de Identificación: </label>
                                    <select value={this.state.tipo_identificacion} onChange={this.handleChangeIden} className="form-control" name="tipo_identificacion">
                            ...
                            <div className="col">
                                <div className="form-group">
                                    ...
                                    <input type="text" id="nro_identificacion" className="form-control" value={this.state.nro_identificacion} onChange={this.handleChangeUpperCase} name="nro_identificacion" style={{ width: '67%', display: 'inline' }} />

                                    {this.state.createstate &&
                                        < button type="button" className="btn btn-outline-primary" onClick={this.consultarWS} style={{ marginTop: '-0.5%', marginLeft: '1px' }}>WS</button>
                                    }
                        ...
                        <div className="row">
                            <div className="col">
                                <div className="form-group">
                                    ...
                                    <select value={this.state.parentesco} onChange={this.handleChange} className="form-control" name="parentesco">
                                        <option value="">Seleccione...</option>
                                        {this.getFormSelectParentesco()}
                                    </select>
                                    <span style={{ color: "red" }}>{this.state.errores["parentesco"]}</span>
                                </div>
                            </div>
                            <div className="col">
                                <div className="form-group">
                        ...
                        <div className="row">
                            <div className="col">
                                <div className="form-group checkbox" style={{ display: 'inline-flex', marginTop: '32px' }}>
                                    <label htmlFor="por_sustitucion" style={{ width: '285px' }}>Por Sustitución?: </label>
                                    <input type="checkbox" id="por_sustitucion" className="form-control" checked={this.state.por_sustitucion} onChange={this.handleInputChange} name="por_sustitucion" style={{ marginTop: '5px', marginLeft: '-90%' }} />
                                </div>
                            </div>
                            <div className="col">
                                <div className="form-group checkbox" style={{ display: 'inline-flex', marginTop: '32px' }}>
                                    <label htmlFor="discapacidad" style={{ width: '285px' }}>Discapacidad?: </label>
                                    <input type="checkbox" id="discapacidad" className="form-control" checked={this.state.discapacidad} onChange={this.handleInputChange} name="discapacidad" style={{ marginTop: '5px', marginLeft: '-90%' }} />
                                </div>
                            </div>
                        </div>
                        <div className="row" style={{ visibility: this.state.discapacidad == true ? 'visible' : 'hidden' }}>
                            <div className="col">
                                <div className="form-group">
                                    <label htmlFor="tipo_dis">Tipo de Discapacidad: </label>
                                    <select value={this.state.tipo_dis} onChange={this.handleChange} className="form-control" name="tipo_dis">
                                        <option value="">Seleccione...</option>
                                        {this.getFormSelectTipoDiscapacidad()}
                                    </select>
                                    <span style={{ color: "red" }}>{this.state.errores["tipo_dis"]}</span>
                                </div>
                            </div>
                            <div className="col">
                                <div className="form-group">
                                    <label htmlFor="porcentaje_dis">Porcentaje Discapacidad (%): </label>
                                    <input type="number" id="porcentaje_dis" className="form-control" value={this.state.porcentaje_dis} onChange={this.handleChange} name="porcentaje_dis" />
                                    <span style={{ color: "red" }}>{this.state.errores["porcentaje_dis"]}</span>
                                </div>
                            </div>
                        </div>
                    </Dialog>
    ...
    generateButton(cell, row) {
        ...
        return (
            <div>
                <button onClick={() => this.LoadCarga(Id)} type="button" className="btn btn-outline-primary btn-sm" style={{ marginLeft: '0.2em' }}>Editar</button>
    ...
    Delete(id) {
        ...
        let arrayCargas = this.state.cargas.slice();
        ...
        axios.post("/RRHH/Colaboradores/DeleteCargaSocialApi/", {
            Id: id
        })
            .then((response) => {
                if (response.data == "OK") {
    ...
    LoadCarga(id) {
        ...
        this.state.fecha_matrimonio = carga[0].fecha_matrimonio == null ? '' : carga[0].fecha_matrimonio;
        this.state.discapacidad = carga[0].discapacidad === null ? false : carga[0].discapacidad;
        this.state.tipo_dis = carga[0].catalogo_tipo_discapacidad_id == null ? '' : carga[0].catalogo_tipo_discapacidad_id;
        this.state.porcentaje_dis = carga[0].catalogo_porcentaje_id == null ? '' : carga[0].catalogo_porcentaje_id;
        this.state.por_sustitucion = carga[0].por_sustitucion === null ? false : carga[0].por_sustitucion;
        this.state.viene_registro_civil = carga[0].viene_registro_civil === null ? false : carga[0].viene_registro_civil;
        this.saveEdad();
    ...
    handleValidation() {
        ...
        if (!this.state.parentesco) {
            ...
            errors["parentesco"] = <div className="alert alert-danger">El campo Parentesco es obligatorio.</div>;
        } else {
            var p = this.state.tiposParentesco.filter(c => c.Id == Number.parseInt(this.state.parentesco));

            if (p != '') {
                var codigo = p[0].codigo.replace(/ /g, "");
                if (codigo == PARENTESCO_CONYUGE || codigo == PARENTESCO_CONVIVIENTE) {
                    var conyuge = this.state.tiposParentesco.filter(c => c.codigo == PARENTESCO_CONYUGE);
                    var conviviente = this.state.tiposParentesco.filter(c => c.codigo == PARENTESCO_CONVIVIENTE);
                    var existe = false;
                    this.state.cargas.forEach(e => {
                        if (e.parentesco_id == conyuge[0].Id || e.parentesco_id == conviviente[0].Id) {
                            existe = true;
                        }
                    });
                    ...
                    if (existe == true) {
                        this.state.formIsValid = false;
                        errors["parentesco"] = <div className="alert alert-danger">No se permite ingresar más de un Cónyuge o Conviviente</div>;
                    }
                }
        ...
        if (!this.state.primer_apellido) {
            this.state.formIsValid = false;
            errors["primer_apellido"] = <div className="alert alert-danger">El campo Primer Apellido es obligatorio.</div>;
        }
        if (!this.state.segundo_apellido) {
            this.state.formIsValid = false;
            errors["segundo_apellido"] = <div className="alert alert-danger">El campo Segundo Apellido es obligatorio.</div>;
        }
        if (!this.state.nombres) {
            this.state.formIsValid = false;
            errors["nombres"] = <div className="alert alert-danger">El campo Nombres es obligatorio.</div>;
        }
        if (this.state.primer_apellido && this.state.nombres && this.state.nombres_apellidos) {
            ...
            if (this.state.nombres_apellidos.replace(/ /g, "") != espacios) {
                this.state.formIsValid = false;
                abp.notify.error('Apellidos o nombres no coinciden con datos de Registro Civil', 'Error');
            }
        ...
        if (!this.state.genero) {
            this.state.formIsValid = false;
            errors["genero"] = <div className="alert alert-danger">El campo Género es obligatorio.</div>;
        }
        if (!this.state.fecha_nacimiento) {
            this.state.formIsValid = false;
            errors["fecha_nacimiento"] = <div className="alert alert-danger">El campo Fecha de Nacimiento es obligatorio.</div>;
        } else {
            var fecha = moment().format("YYYY-MM-DD");
            if (this.state.fecha_nacimiento > fecha) {
                this.state.formIsValid = false;
                errors["fecha_nacimiento"] = <div className="alert alert-danger">La fecha ingresada no puede ser mayor a la fecha actual</div>;
            }
            /*if (p != '') {
                console.log(p)
                if (p[0].codigo == PARENTESCO_HIJO && this.state.edad >= 18 && this.state.discapacidad == false) {
                    // this.state.formIsValid = false;
                    errors["fecha_nacimiento"] = <div className="alert alert-danger">Hijo mayor de edad</div>;
                }
                // else if (p[0].codigo == PARENTESCO_HIJO && this.state.edad >= 18 && this.state.discapacidad == true) {
                //     if (!this.state.tipo_dis) {
                //         this.state.formIsValid = false;
                //         errors["tipo_dis"] =<div className="alert alert-danger">El campo Tipo de Discapacidad es obligatorio.</div>;
                //     }
                //     if (!this.state.porcentaje_dis) {
                //         this.state.formIsValid = false;
                //         errors["porcentaje_dis"] =<div className="alert alert-danger">El campo Porcentaje Discapacidad es obligatorio.</div>;
                //     }
                // }
        ...
        if (!this.state.pais) {
            this.state.formIsValid = false;
            errors["pais"] = <div className="alert alert-danger">El campo País de Nacimiento es obligatorio.</div>;
        }
        if (!this.state.nacionalidad) {
            this.state.formIsValid = false;
            errors["nacionalidad"] = <div className="alert alert-danger">El campo Nacionalidad es obligatorio.</div>;
        }
        if (this.state.estado_civil) {
            ...
            if (catEstadoCivil[0].codigo == ESTADO_CIVIL_CASADO && !this.state.fecha_matrimonio) {
                this.state.formIsValid = false;
                errors["fecha_matrimonio"] = <div className="alert alert-danger">El campo Fecha de Matrimonio es obligatorio.</div>;
            } else if (catEstadoCivil[0].codigo == ESTADO_CIVIL_CASADO && this.state.fecha_matrimonio > today) {
                this.state.formIsValid = false;
                errors["fecha_matrimonio"] = <div className="alert alert-danger">Fecha de Matrimonio no puede ser mayor a fecha actual.</div>;
            }
            /*if (p != '') {
                if (p[0].codigo == PARENTESCO_HIJO && catEstadoCivil[0].codigo == ESTADO_CIVIL_CASADO) {
                    this.state.formIsValid = false;
            ...
            this.state.formIsValid = false;
            errors["estado_civil"] = <div className="alert alert-danger">El campo Estado Civil es obligatorio.</div>;
        }
        if (this.state.discapacidad == true) {
            ...
            if (!this.state.porcentaje_dis) {
                ...
                if (this.state.porcentaje_dis > 100 || this.state.porcentaje_dis <= 0) {
                    this.state.formIsValid = false;
                    errors["porcentaje_dis"] = <div className="alert alert-danger">Debe ingresar un valor entero entre 1 y 100.</div>;
                }
            }
        }
        if (this.state.por_sustitucion == true) {
            if (p != '') {
                if (p[0].codigo == PARENTESCO_HIJO && this.state.discapacidad == false) {
    ...
    Guardar() {
        ...
        if (this.state.formIsValid) {
            ...
            axios.post("/RRHH/Colaboradores/CreateCargaSocialApi/", {
                tipoIdentificacion: this.state.id_carga,
    ...
    AlmacenarCargaSocia(id) {
        ...
        let cargas = {};
        var tipoIdent = this.state.tiposIdentificacion.filter(c => c.Id === parseInt(this.state.tipo_identificacion));
        var par = this.state.tiposParentesco.filter(c => c.Id === parseInt(this.state.parentesco));
        var genero = this.state.generos.filter(c => c.Id === parseInt(this.state.genero));
        // console.log(genero)
        // console.log('tipoIdent', tipoIdent);
        // console.log(this.state.id_carga);
        if (this.state.id_carga != 0) {
            console.log('existente', this.state.cargas);
            cargas = this.state.cargas.filter(c => c.id_carga === this.state.id_carga);
            var indexCarga = this.state.cargas.findIndex(c => c.Id === this.state.id_carga);
            if (indexCarga < 0) {
                indexCarga = 0;
            }
            if (cargas.length == 0) {
                cargas = this.state.cargas.filter(c => c.idCarga === this.state.id_carga);
            }
            ...
            cargas.apellidos = this.state.primer_apellido + ' ' + this.state.segundo_apellido;
            ...
            cargas.nombre_sustituto = this.state.por_sustitucion == true ? "SI" : "NO";
            ...
            cargas = {
                ...
                catalogo_tipo_discapacidad_id: this.state.tipo_dis,
                catalogo_porcentaje_id: this.state.porcentaje_dis,
                apellidos: this.state.primer_apellido + ' ' + this.state.segundo_apellido,
                por_sustitucion: this.state.por_sustitucion,
    ...
    GetCatalogos() {
        ...
        codigos = ['PARENTESCO', 'TIPOINDENTIFICACION', 'GENERO', 'ESTADOCIVIL', 'TIPODISCAPACIDAD',
            'PORCENTAJEDISCAPACIDAD', 'NACIONALIDADES'];
        ...
        data.forEach(e => {
            ...
            switch (codigoCatalogo) {
                ...
                case 'PORCENTAJEDISCAPACIDAD':
                    this.setState({ porcentajeDis: catalogo })
                    this.getFormSelectPorcentaje()
                    return;
            }
    ...
    getFormSelectParentesco() {
        return (
            this.state.tiposParentesco.map((item) => {
                return (
                    <option key={Math.random()} value={item.Id}>{item.nombre}</option>
                )
            })
    ...
    getFormSelectPorcentaje() {
        return (
            this.state.porcentajeDis.map((item) => {
                return (
                    <option key={Math.random()} value={item.Id}>{item.nombre}</option>
                )
            })
