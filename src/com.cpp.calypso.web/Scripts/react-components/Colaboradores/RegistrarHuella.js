...
export default class RegistrarHuella extends React.Component {
    ...
    CargarHuella() {
        if (this.props.id_huella != 0) {
            ...
            axios.post("/RRHH/Colaboradores/GetHuellaApi/" + this.props.id_huella, {})
                .then((response) => {
    ...
    Guardar() {
        ...
        if (this.state.formIsValid == true) {
            ...
            if (this.state.Id == 0) {
                axios.post("/RRHH/Colaboradores/CrearHuellaApiAsync/", {
                    Id: this.state.Id,
                ...
                axios.post("/RRHH/Colaboradores/UpdateHuellaApiAsync/", {
                    Id: this.state.Id,
                    dedo: this.state.dedo,
                    colaborador: this.props.id_colaborador,
                    huella: this.state.huella,
    ...
    CargarCatalogos(data) {
        data.forEach(e => {
            ...
            if (codigoCatalogo == 'DEDOS') {
                this.setState({ dedos: catalogo })
                this.getFormSelectDedos();
                return;
            }
        });
