...
export default class EditarInformacionGeneralReingreso extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      id_candidato: "",
  ...
  render() {
    let Reingreso = sessionStorage.getItem("esReingreso");
    console.log("Reingreso", Reingreso);
    const footer = (
      <div>
  ...
  handleValidation() {
    ...
    /*if (!this.state.empleado_id_sap_local) {
			...
			errors['empleado_id_sap_local'] = (
				<div className="alert alert-danger">El campo ID SAP Local es obligatorio</div>
			);
		}*/
    ...
    if (!this.state.fecha_ingreso) {
      ...
      axios
      .post("/RRHH/Colaboradores/GetValidarFechaPeriodoRegistrosAnterioresReingreso/", {
  ...
  AbrirConfirmacion = () => {
    let self = this;
    abp.message.confirm(
      "Este proceso reintegrará al Colaborador seleccionado en estado Temporal, una vez completada la informacion General podra editar todas las opciones adicionales",
      "¿Está seguro?",
  ...
  Guardar() {
    console.log("GuardaReingreso");
    const IdAntiguo = sessionStorage.getItem("id_colaborador");
    this.handleValidation();
    ...
    if (this.state.formIsValid == true) {
      ...
      this.props.colaborador.empleado_id_sap_local = this.state.empleado_id_sap_local;
      ...
      console.log("Colaborador a Reingresar", this.props.colaborador);
      axios
        .post("/RRHH/Colaboradores/CreateReingresoApi/", {
          colaborador: this.props.colaborador,
          IdAntiguo: IdAntiguo,
        })
        .then((response) => {
          console.log("Result Reingreso", response.data);
          if (response.data.success === "OK") {
            this.cerrarConfirmacion();

            abp.notify.success(
              "Colaborador Reingresado!, complete la información y termine el proceso",
              "Aviso"
            );
            ...
            this.props.cambiarIdColaboradorSession(
              response.data.nuevoReingresoId,
              IdAntiguo
            );
            this.props.activeTabs(false);

            this.setState({ loading: false });
            this.LoadColaboradorEdit(response.data.nuevoReingresoId);

            //this.props.ConsultaColaborador();
          } else {
            abp.notify.error(response.data.success, "Ocurrió un Error");
            this.setState({ loading: false });
          }
        })
        .catch((error) => {
