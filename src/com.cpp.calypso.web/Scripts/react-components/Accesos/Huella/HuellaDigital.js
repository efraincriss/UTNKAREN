...
export default class HuellaDigital extends React.Component {
    ...
    GetHuellas(id) {
        axios.post("/Accesos/Huella/GetHuellasPorColaboradorApi/" + id, {})
            .then((response) => {
    ...
    generateButton(cell, row) {
        return (
            <div style={{ textAlign: 'center' }}>
                ...
                <button onClick={() => { if (window.confirm('Está seguro?')) this.Delete(row.Id) }} className="btn btn-outline-danger btn-sm fa" style={{ marginLeft: '0.2em' }}>Eliminar</button>
            </div>
        )
    }

    Delete(id) {
        axios.post("/Accesos/Huella/DeleteHuellaApi/" + id, {})
            .then((response) => {
                this.GetHuellas(this.state.Id);
    ...
    buscarColaborador() {
        ...
        axios.post("/Accesos/Huella/GetNamesHuella/" + id, {})
            .then((response) => {
    ...
    Regresar() {
        ...
        return (
            window.location.href = currentLocation.origin + "/Accesos/Huella/Vista/"
        );

    }
}
const Container = Wrapper(HuellaDigital);
ReactDOM.render(
    <Container />,
