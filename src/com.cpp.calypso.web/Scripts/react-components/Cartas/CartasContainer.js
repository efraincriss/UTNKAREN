...
class Cartas extends React.Component {
  ...
  isValid = () => {
    ...
    if (this.state.EmpresaId == 0) {
      errors.EmpresaId = "Campo Requerido";
    }
  ...
  GetCatalogs = () => {
    ...
    axios
      .post("/Proyecto/Carta/ObtenerListCarta", {})
      .then(response => {
        console.clear();
        console.log("List Cartas", response.data);
        var datos = response.data.map(item => {
          return {
            label: item.numero_carta + " - " + item.asunto,
            dataKey: item.Id,
            value: item.Id
          };
        });

        this.setState({ listcartas: datos });
        this.state.listcartas.unshift({
          label: "Seleccione..",
          dataKey: 0,
          value: 0
        });
  ...
  render() {
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        ...
        <Dialog
          header={this.state.numero_carta}
